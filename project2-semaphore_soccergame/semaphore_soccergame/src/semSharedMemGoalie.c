...
static int goalieConstituteTeam (int id)
{
    ...
    sh->fSt.goaliesFree++;      //os q vao entrar nas equipas
    sh->fSt.goaliesArrived++;   //os q chegam

    if (sh->fSt.goaliesArrived < NUMGOALIES){
        if (sh->fSt.playersFree >= NUMTEAMPLAYERS){ //capitao goalie  
    ...
    if (sh->fSt.st.goalieStat[id] == WAITING_TEAM){
        ...
        ret = sh->fSt.teamId; // teamId do goalie
